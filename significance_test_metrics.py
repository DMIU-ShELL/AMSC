...
CT-graph, where task performance is already in [0, 1]:
  ...
  Relative average forgetting:
    python significance_test_metrics.py --ref Mask-SC-2 \\
      --methods Mask-SC-2=<masksc_log_root> Mask-LC=<masklc_log_root> \\
      --task-order-config env_configs/ct28/seed1/meta_ctgraph_ct28_interleaved.json \\
      --metric rel_forgetting --tests all --alternative less
...
def main():
    ...
    ap.add_argument(
        "--metric",
        choices=["eval", "auc", "auc_global", "tpot", "fwt", "forgetting", "rel_forgetting", "bwt"],
        default="eval",
    )
    ...
    for name, root in methods.items():
        ...
        for rd in runs:
            ...
            if args.metric == "eval":
                val = float(norm_mat[-1, :].sum())
            elif args.metric in ("auc", "auc_global", "tpot"):
                if args.metric == "auc":
                    val = float(
                        np.nanmean(
                            normalized_per_task_auc(
                                mat, return_scales, clip=clip_norm
                            )
                        )
                    )
                else:
                    val = float(np.trapz(norm_mat.sum(axis=1), dx=1.0))
                ...
                val = compute_forgetting(norm_mat)
            elif args.metric == "rel_forgetting":
                val = compute_rel_forgetting(norm_mat)
            else:
                val = compute_bwt(norm_mat)
            vals.append(val)
            if seed is not None:
